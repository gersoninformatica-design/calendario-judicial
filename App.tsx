...
const App: React.FC = () => {
  ...
  // LA LLAVE MAESTRA DE GERSON
  const ADMIN_EMAIL = 'gerson.informatica@gmail.com';
  
  // Detección inmediata de Gerson por correo de sesión
  ...
  const fetchOrCreateProfile = async (userId: string, email: string | undefined, fullName: string | undefined) => {
    try {
      ...
      const isUserGerson = normalizedEmail === ADMIN_EMAIL;
      ...
      // Si el perfil no existe o es Gerson y no está marcado como aprobado en DB, forzar/crear
      if (!profileData && email) {
        const { data: newProfile } = await supabase
          ...
          .insert({
            ...
            is_approved: isUserGerson // Auto-aprobación técnica
          })
          .select()
          .maybeSingle();
        profileData = newProfile;
      } else if (profileData && isUserGerson && !profileData.is_approved) {
        // Failsafe: Si Gerson existe pero no está aprobado, lo aprobamos
        const { data: updatedProfile } = await supabase
          .from('profiles')
          .update({ is_approved: true, role: 'Administrador de Sistemas' })
          .eq('id', userId)
          .select()
          .maybeSingle();
  ...
  useEffect(() => {
    if (!session || (!profile?.is_approved && !isGersonAdmin)) return;
    
    const fetchData = async () => {
      try {
        const [{ data: unitsData }, { data: eventsData }] = await Promise.all([
          supabase.from('units').select('*').order('name'),
          supabase.from('events').select('*').order('startTime')
        ]);
        setUnits(unitsData || []);
  ...
  if (!isLoaded) return (
    <div className="h-screen w-full flex flex-col items-center justify-center bg-slate-900">
      ...
      <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">Verificando Credenciales Judiciales...</p>
    </div>
  );
  ...
  // PANTALLA DE BLOQUEO (Bypass absoluto para Gerson)
  if (!isGersonAdmin && profile && !profile.is_approved) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-slate-50 p-6">
        <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border-4 border-slate-100 max-w-md w-full text-center animate-in zoom-in-95">
           ...
           <p className="text-slate-500 font-bold mb-8 text-sm px-4">
             Hola <span className="text-blue-600 font-black">{profile.full_name || 'Funcionario'}</span>. Tu cuenta está registrada, pero el administrador <span className="text-slate-900">Gerson Informatica</span> debe autorizar tu ingreso.
           </p>
           ...
           <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest text-center">SOPORTE: GERSON INFORMATICA</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-2xl shadow-sm">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-600">
              {isGersonAdmin ? 'CONSOLA MAESTRA (ADMINISTRADOR)' : `FUNCIONARIO: ${profile?.full_name}`}
            </span>
          </div>
          <div className="flex gap-3">
             ...
             <button onClick={() => { setIsAnalyzing(true); analyzeSchedule(eventsWithUnit).then(res => { alert(res); setIsAnalyzing(false); }); }} className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 text-white text-[10px] font-black uppercase rounded-2xl shadow-lg hover:shadow-indigo-200 transition-all">
                {isAnalyzing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />} IA Judicial
             </button>
          </div>
        </div>
